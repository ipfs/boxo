...
> [!NOTE]
> This document describes boxo v0.42.1, the version used by Kubo v0.43. If you
> run a different version, double check the details below against that
> version's source and godoc, as the implementation may change over time.
...
The overall logic works by sending to non-local peers only if those peers have previously replied that they have data blocks. To minimize impact on existing workloads, by default, broadcasts are still always sent to peers on the local network, or the ones defined in Kubo's `Peering.Peers` config.


## Testing with Kubo

At Shipyard, we conducted A/B testing on our internal Kubo staging gateway with organic CID requests to `ipfs.io`. While these results may not exactly match your specific workload, the benefits proved significant enough to make this feature default in Kubo v0.36. Here are the key findings:
...
For more information about our A/B tests, see [ipfs/kubo#10825](https://github.com/ipfs/kubo/pull/10825).
...
For a complete client setup with broadcast reduction enabled, see `ExampleNew_broadcastControl` in [`bitswap/client/example_test.go`](../bitswap/client/example_test.go).
