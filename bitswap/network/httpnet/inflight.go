...
//
// On a successful round trip, statusCode, body, latency, retryAfter, and
...
func (t *inflightTracker) do(key string, fn func() *inflightResult) (result *inflightResult, shared bool) {
	...
	// Deferred so that a panicking fn cannot leave the key claimed
	// forever with waiters blocked on done.
	defer func() {
		t.mu.Lock()
		delete(t.calls, key)
		t.mu.Unlock()
