...
func (s *streamMessageSender) Connect(ctx context.Context) (network.Stream, error) {
	...
	if err := s.bsnet.Connect(ctx, s.to); err != nil {
		return nil, err
	}
...
func (bsnet *impl) Ping(ctx context.Context, p peer.AddrInfo) ping.Result {
	ctx, cancel := context.WithCancel(ctx)
	defer cancel()
	res := <-ping.Ping(ctx, bsnet.host, p.ID)
	return res
}

func (bsnet *impl) Latency(p peer.AddrInfo) time.Duration {
	return bsnet.host.Peerstore().LatencyEWMA(p.ID)
}
...
func (bsnet *impl) NewMessageSender(ctx context.Context, p peer.AddrInfo, opts *iface.MessageSenderOpts) (iface.MessageSender, error) {
...
func (bsnet *impl) newStreamToPeer(ctx context.Context, p peer.AddrInfo) (network.Stream, error) {
	return bsnet.host.NewStream(ctx, p.ID, bsnet.supportedProtocols...)
}
...
func (bsnet *impl) handleNewStream(s network.Stream) {
	...
	for {
		...
		p := peer.AddrInfo{
			ID:    rpid,
			Addrs: bsnet.host.Peerstore().Addrs(rpid),
		}
...
func (bsnet *impl) TagPeer(p peer.AddrInfo, tag string, w int) {
	bsnet.host.ConnManager().TagPeer(p.ID, tag, w)
}

func (bsnet *impl) UntagPeer(p peer.AddrInfo, tag string) {
	bsnet.host.ConnManager().UntagPeer(p.ID, tag)
}

func (bsnet *impl) Protect(p peer.AddrInfo, tag string) {
	bsnet.host.ConnManager().Protect(p.ID, tag)
}

func (bsnet *impl) Unprotect(p peer.AddrInfo, tag string) bool {
	return bsnet.host.ConnManager().Unprotect(p.ID, tag)
}
...
func (nn *netNotifiee) Connected(n network.Network, v network.Conn) {
	...
	p := peer.AddrInfo{
		ID:    rpid,
		Addrs: nn.impl().host.Peerstore().Addrs(rpid),
	}
	nn.impl().connectEvtMgr.Connected(p)
}
...
func (nn *netNotifiee) Disconnected(n network.Network, v network.Conn) {
	...
	nn.impl().connectEvtMgr.Disconnected(p)
}
