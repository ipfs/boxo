...
func TestBufferedPinProvider(t *testing.T) {
	...
	// The second keyChan should contain both root and root2 pins, while the
	// first keyChan contains only the elements from the first pin because they
	// were all cached before the second pin happened.
	root2count := 0
	for range root2pins {
		root2count++
	}
