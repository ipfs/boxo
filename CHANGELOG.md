...
- `ipns`: `NewRecord` accepts `WithMetadata` option for storing custom scalar key-value pairs (string, bytes, int64, bool) in the signed DAG-CBOR data of IPNS records. Metadata can be read back from unmarshaled records via `Record.Metadata` and iterated with `Record.MetadataEntries`. Reserved IPNS field names and empty keys are rejected. [#1085](https://github.com/ipfs/boxo/pull/1085)
