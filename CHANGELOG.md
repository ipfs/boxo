...
* `gateway` now supports `car-dups`, `car-order` and `car-version` as query parameters in addition to the `Accept` header parameters. The parameters in the `Accept` header have always priority.
