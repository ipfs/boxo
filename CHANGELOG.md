...
- 🛠 `routing/offline`: the offline router now stores value records in the same datastore layout as the go-libp2p-kad-dht value store (v0.42.0+), so a node sharing one datastore between the offline router and a DHT resolves records offline that were published online and vice versa. That interop previously relied on both sides coincidentally using the same key derivation, which the kad-dht v0.42.0 layout change broke. Records written in the old layout (root-level base32 keys) are no longer read; `GetValue` also returns `routing.ErrNotFound` instead of leaking `datastore.ErrNotFound` for missing records, and re-validates stored records on read so expired records (e.g. IPNS past its EOL) are no longer returned.
- upgrade to `go-libp2p-kad-dht` [v0.42.0](https://github.com/libp2p/go-libp2p-kad-dht/releases/tag/v0.42.0)+ (pre-release pin including the [local record validation fix](https://github.com/libp2p/go-libp2p-kad-dht/pull/1285))
