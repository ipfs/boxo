...
// testnet
func NewTestInstanceGenerator(net tn.Network, routing mockrouting.Server, netOptions []bsnet.NetOpt, bsOptions []blockexchange.Option) InstanceGenerator {
...
type InstanceGenerator struct {
	...
	bsOptions  []blockexchange.Option
...
func NewInstance(ctx context.Context, net tn.Network, router routing.Routing, p tnet.Identity, netOptions []bsnet.NetOpt, bsOptions []blockexchange.Option) Instance {
	...
	bs := blockexchange.New(ctx, adapter, router, bstore, bsOptions...)
	return Instance{
		Datastore:       ds,
		Adapter:         adapter,
		Identity:        p,
