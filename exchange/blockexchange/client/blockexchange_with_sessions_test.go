...
import (
	...
	"github.com/ipfs/boxo/exchange/blockexchange/client/internal/session"
	"github.com/ipfs/boxo/exchange/blockexchange/client/traceability"
...
// PQM when creating BlockExchange.
func TestCustomProviderQueryManager(t *testing.T) {
	...
	// Replace BlockExchange in instance a with our customized one.
	...
	bs := blockexchange.New(ctx, a.Adapter, pqm, a.Blockstore,
		blockexchange.WithClientOption(client.WithDefaultProviderQueryManager(false)))
	a.Exchange.Close() // close old to be sure.
	a.Exchange = bs
	// Connect instances only after BlockExchange exists.
...
func TestSessionBetweenPeers(t *testing.T) {
	...
	router := mockrouting.NewServer()
	ig := testinstance.NewTestInstanceGenerator(vnet, router, nil, []blockexchange.Option{blockexchange.SetSimulateDontHavesOnTimeout(false)})
	defer ig.Close()
...
func TestFetchNotConnected(t *testing.T) {
	...
	vnet := getVirtualNetwork()
	router := mockrouting.NewServer()
	ig := testinstance.NewTestInstanceGenerator(vnet, router, nil, []blockexchange.Option{blockexchange.ProviderSearchDelay(10 * time.Millisecond)})
	defer ig.Close()
...
func TestFetchAfterDisconnect(t *testing.T) {
	...
	vnet := getVirtualNetwork()
	router := mockrouting.NewServer()
	ig := testinstance.NewTestInstanceGenerator(vnet, router, nil, []blockexchange.Option{
		blockexchange.ProviderSearchDelay(10 * time.Millisecond),
		blockexchange.RebroadcastDelay(delay.Fixed(15 * time.Millisecond)),
	})
	defer ig.Close()
...
func TestDontHaveTimeoutConfig(t *testing.T) {
	...
	bs := blockexchange.New(ctx, a.Adapter, pqm, a.Blockstore,
		blockexchange.WithClientOption(client.WithDontHaveTimeoutConfig(cfg)))
	a.Exchange.Close()
	a.Exchange = bs
