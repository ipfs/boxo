...
const (
	...
	// default BlockExchange client. 0 value means unlimited.
	BlockExchangeClientDefaultMaxProviders = 10
	// Number of concurrent workers in decision engine that process requests to the blockstore
	BlockExchangeEngineBlockstoreWorkerCount = 128
	// the total number of simultaneous threads sending outgoing messages
	BlockExchangeTaskWorkerCount = 8
	// how many worker threads to start for decision engine task worker
	BlockExchangeEngineTaskWorkerCount = 8
	// the total amount of bytes that a peer should have outstanding, it is utilized by the decision engine
	BlockExchangeMaxOutstandingBytesPerPeer = 1 << 20
	// the number of bytes we attempt to make each outgoing BlockExchange message
	BlockExchangeEngineTargetMessageSize = 16 * 1024
