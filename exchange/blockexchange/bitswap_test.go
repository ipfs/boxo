...
func TestDoesNotProvideWhenConfiguredNotTo(t *testing.T) {
	net := tn.VirtualNetwork(delay.Fixed(kNetworkDelay))
	block := blocks.NewBlock([]byte("block"))
	bsOpts := []blockexchange.Option{blockexchange.ProviderSearchDelay(50 * time.Millisecond)}
	router := mockrouting.NewServer()
...
func PerformDistributionTest(t *testing.T, numInstances, numBlocks int) {
	...
	net := tn.VirtualNetwork(delay.Fixed(kNetworkDelay))
	router := mockrouting.NewServer()
	ig := testinstance.NewTestInstanceGenerator(net, router, nil, []blockexchange.Option{
		blockexchange.TaskWorkerCount(5),
		blockexchange.EngineTaskWorkerCount(5),
		blockexchange.MaxOutstandingBytesPerPeer(1 << 20),
	})
	defer ig.Close()
...
func assertStat(t *testing.T, st *blockexchange.Stat, sblks, rblks, sdata, rdata uint64) {
...
func TestWithScoreLedger(t *testing.T) {
	...
	net := tn.VirtualNetwork(delay.Fixed(kNetworkDelay))
	router := mockrouting.NewServer()
	bsOpts := []blockexchange.Option{blockexchange.WithScoreLedger(tsl)}
	ig := testinstance.NewTestInstanceGenerator(net, router, nil, bsOpts)
	defer ig.Close()
	i := ig.Next()
