...
func newMockAPI(t *testing.T) (*mockAPI, cid.Cid) {
	...
	n := mockNamesys{}
	gwApi, err := NewBlocksGateway(blockService, n)
	if err != nil {
		t.Fatal(err)
	}
...
func (api *mockAPI) Get(ctx context.Context, immutablePath ImmutablePath, opt ...GetOpt) (ContentPathMetadata, files.Node, error) {
	return api.gw.Get(ctx, immutablePath, opt...)
}
...
func TestPretty404(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{"/nope", "application/json", http.StatusNotFound, fmt.Sprintf("could not fetch content at path /ipns/example.net/nope: no link named \"nope\" under %s\n", k.Cid().String())},
...
// TODO: better constructor that can take configurable namesys and router
func NewBlocksGateway(blockService blockservice.BlockService, ns namesys.NameSystem) (*BlocksGateway, error) {
...
func (api *BlocksGateway) getPathRoots(ctx context.Context, contentPath ImmutablePath) ([]cid.Cid, ipath.Resolved, error) {
	/*
	...
	*/
	...
	for _, root := range pathSegments {
		...
		sp.WriteString("/")
		sp.WriteString(root)
		resolvedSubPath, err := api.ResolvePath(ctx, ipath.New(sp.String()))
		if err != nil {
			return nil, nil, err
		}
...
func (api *BlocksGateway) ResolveMutable(ctx context.Context, p ipath.Path) (ImmutablePath, error) {
	err := p.IsValid()
	if err != nil {
		return ImmutablePath{}, err
	}

	ip := path.Path(p.String())
	switch ip.Segments()[0] {
	case "ipns":
		ip, err = resolve.ResolveIPNS(ctx, api.namesys, ip)
		if err != nil {
			return ImmutablePath{}, err
		}
		imPath, err := NewImmutablePath(ipath.New(ip.String()))
		if err != nil {
			return ImmutablePath{}, err
		}
		return imPath, nil
...
func (api *BlocksGateway) GetDNSLinkRecord(ctx context.Context, hostname string) (ipath.Path, error) {
	if api.namesys != nil {
		p, err := api.namesys.Resolve(ctx, "/ipns/"+hostname, nsopts.Depth(1))
		if err == namesys.ErrResolveRecursion {
			err = nil
...
func (api *BlocksGateway) IsCached(ctx context.Context, p ipath.Path) bool {
...
func (api *BlocksGateway) ResolvePath(ctx context.Context, p ipath.Path) (ipath.Resolved, error) {
...
func (api *BlocksGateway) resolveNode(ctx context.Context, p ipath.Path) (format.Node, error) {
	rp, err := api.ResolvePath(ctx, p)
	if err != nil {
		return nil, err
	}
